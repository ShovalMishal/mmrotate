...
def get_cls_results_for_ood(det_results, annotations, class_id, box_type):
    """Get det results and gt information of a certain class.
    ...
    Returns:
        tuple[list[np.ndarray]]: detected bboxes, gt bboxes, ignored gt bboxes
    """
    cls_dets = [np.concatenate(img_res) for img_res in det_results]
...
def eval_ood_rbbox_recalls(det_results,
                           ...
                           logger=None,
                           nproc=4):
    """Evaluate mAP of a rotated dataset, for ood classes.
    ...
    """
    assert len(det_results) == len(annotations)
    ood_classes = ['container-crane', 'airport', 'helipad']
    num_imgs = len(det_results)
    num_scales = len(scale_ranges) if scale_ranges is not None else 1
    ...
    pool = get_context('spawn').Pool(nproc)
    eval_results = []
    for i in range(15, 15+num_ood_classes):
...
def print_recall_summary(mean_recall,
    ...
    for i in range(num_scales):
        ...
        table_data.append(['mAP', '', '', '', f'{mean_recall[i]:.3f}'])
